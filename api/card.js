...
function decodeEntities(text) {
    return text.replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&apos;/g, "'").replace(/&#x([0-9a-fA-F]+);/gi, (_, h) => String.fromCharCode(parseInt(h, 16))).replace(/&#(\d+);/g, (_, c) => String.fromCharCode(parseInt(c, 10)));
}
...
function httpGet(hostname, path) {
    return new Promise((resolve) => {
        const req = https.request({ hostname, path, method: "GET", headers: { "User-Agent": "profile-card-generator" } }, (res) => {
            let data = "";
            res.on("data", (c) => (data += c));
...
async function fetchSpotifyData() {
    ...
    const data = await httpGet("spotify-github-profile.kittinanx.com", `/api/view?uid=${spotifyUserId}`);

    if (!data || /class="not-play"/.test(data)) {
        return fallback;
    };
...
function calculateRank({ commits, pullRequests, issues, reviews, stars, followers }) {
    ...
    const commitsMedian = 250, commitsWeight = 2;
    const prsMedian = 50, prsWeight = 3;
    const issuesMedian = 25, issuesWeight = 1;
    const reviewsMedian = 2, reviewsWeight = 1;
    const starsMedian = 50, starsWeight = 4;
    const followersMedian = 10, followersWeight = 1;
    const totalWeight = commitsWeight + prsWeight + issuesWeight + reviewsWeight + starsWeight + followersWeight;

    const thresholds = [1, 12.5, 25, 37.5, 50, 62.5, 75, 87.5, 100];
    const levels = ["S", "A+", "A", "A-", "B+", "B", "B-", "C+", "C"];

    const rank = 1 - (commitsWeight * expCdf(commits / commitsMedian) + prsWeight * expCdf(pullRequests / prsMedian) + issuesWeight * expCdf(issues / issuesMedian) + reviewsWeight * expCdf(reviews / reviewsMedian) + starsWeight * logNormalCdf(stars / starsMedian) + followersWeight * logNormalCdf(followers / followersMedian)) / totalWeight;
...
function createBottomRow(spotify, viewCount) {
    ...
    const artBlock = albumArt ? `
        ...
        <rect x="${AX - 1}" y="${AY - 1}" width="${AS + 2}" height="${AS + 2}" rx="${AR + 1}" fill="none" stroke="${theme.accent}" stroke-width="1.5" opacity="0.5"/>
        <image href="${albumArt}" x="${AX}" y="${AY}" width="${AS}" height="${AS}" clip-path="url(#ac)" preserveAspectRatio="xMidYMid slice"/>
    ` : "";
    ...
    const eqStyle = `<style>
        ${Array.from({ length: 5 }, (_, i) => {
                ...
                return `@keyframes eq${i}{0%,100%{transform:scaleY(${lo})}50%{transform:scaleY(1)}}` + `.eq${i}{transform-box:fill-box;transform-origin:bottom;animation:eq${i} ${periods[i]}s ease-in-out ${delays[i]}s infinite;}`;
            }).join("")}
        </style>
    `;
