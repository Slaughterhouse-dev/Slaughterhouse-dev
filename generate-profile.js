...
function generateSVG(user, streak, langs, stars, commits, prs, issues, rankPercentile) {
    ...
    // Circle is moved up to leave more breathing room between it and the labels below.
    const scx = 380, scy = 276, sr = 34;
    ...
    return `<svg width="760" height="396" viewBox="0 0 760 396" xmlns="http://www.w3.org/2000/svg" role="img">
        ...
        <text x="${scx}" y="${scy + sr + 32}" ${S.font} font-size="14" fill="${S.accent}" text-anchor="middle">Current Streak</text>
        <text x="${scx}" y="${scy + sr + 50}" ${S.font} font-size="13" fill="${S.muted}"  text-anchor="middle">${fmtDate(streak.startCurrent)} - ${fmtDate(streak.endCurrent)}</text>
...
<text x="${x + 11}" y="${y}" ${S.font} font-size="13" fill="${S.text}">${name} <tspan fill="${S.muted}">${pct}%</tspan></text>`;
}).join("");
}
...
<rect x="16" y="16" width="454" height="188" rx="8" fill="${S.bgCard}" stroke="${S.border}" stroke-width="0.5"/>
<text x="32" y="44" ${S.font} font-size="16" font-weight="600" fill="${S.accent}">S0x2-dev's GitHub Stats</text>

<text x="32"  y="78"  ${S.font} font-size="14" fill="${S.muted}">Total Stars Earned:</text>
<text x="260" y="78"  ${S.font} font-size="14" font-weight="600" fill="${S.text}">${stars}</text>

<text x="32"  y="102" ${S.font} font-size="14" fill="${S.muted}">Total Commits (last year):</text>
<text x="260" y="102" ${S.font} font-size="14" font-weight="600" fill="${S.text}">${fmtNum(commits)}</text>

<text x="32"  y="126" ${S.font} font-size="14" fill="${S.muted}">Total PRs:</text>
<text x="260" y="126" ${S.font} font-size="14" font-weight="600" fill="${S.text}">${prs}</text>

<text x="32"  y="150" ${S.font} font-size="14" fill="${S.muted}">Total Issues:</text>
<text x="260" y="150" ${S.font} font-size="14" font-weight="600" fill="${S.text}">${issues}</text>

<text x="32"  y="174" ${S.font} font-size="14" fill="${S.muted}">Contributed to (last year):</text>
<text x="260" y="174" ${S.font} font-size="14" font-weight="600" fill="${S.text}">${user.repositoriesContributedTo.totalCount}</text>
...
<circle cx="408" cy="112" r="38" fill="none" stroke="${S.accent}" stroke-width="3"
    ...
    stroke-dashoffset="0"
    transform="rotate(-90 408 112)"/>
<text x="408" y="119" ${S.font} font-size="20" font-weight="700" fill="${S.text}" text-anchor="middle">A+</text>

<rect x="482" y="16" width="262" height="188" rx="8" fill="${S.bgCard}" stroke="${S.border}" stroke-width="0.5"/>
<text x="506" y="44" ${S.font} font-size="16" font-weight="600" fill="${S.accent}">Most Used Languages</text>
${legendSVG}
${donutSVG}
...
<text x="137" y="${sideNumY}"   ${S.font} font-size="27" font-weight="700" fill="${S.text}" text-anchor="middle">${total.toLocaleString()}</text>
<text x="137" y="${sideLabelY}" ${S.font} font-size="13" fill="${S.muted}" text-anchor="middle">Total Contributions</text>
<text x="137" y="${sideDateY}"  ${S.font} font-size="12" fill="${S.muted}" text-anchor="middle">Apr 30, 2024 - Present</text>
...
<circle cx="${scx}" cy="${scy}" r="${sr}" fill="none" stroke="${S.accent}" stroke-width="2.5"/>
<text x="${scx}" y="${scy + 10}"      ${S.font} font-size="27" font-weight="700" fill="${S.text}" text-anchor="middle">${streak.current}</text>
<text x="${scx}" y="${scy + sr + 28}" ${S.font} font-size="13" fill="${S.accent}" text-anchor="middle">Current Streak</text>
<text x="${scx}" y="${scy + sr + 43}" ${S.font} font-size="12" fill="${S.muted}"  text-anchor="middle">${fmtDate(streak.startCurrent)} - ${fmtDate(streak.endCurrent)}</text>

<text x="621" y="${sideNumY}"   ${S.font} font-size="27" font-weight="700" fill="${S.text}" text-anchor="middle">${streak.longest}</text>
<text x="621" y="${sideLabelY}" ${S.font} font-size="13" fill="${S.muted}" text-anchor="middle">Longest Streak</text>
</svg>`;
}
